...
struct modules
{
	...
	std::unique_ptr < module::Encoder_conv<>> encoder;
...
void init_params(params& p)
{
	...
	std::cout << "#    ** Encoder_conv " << std::endl;
...
void init_modules(const params& p, modules& m)
{
	...
	m.encoder = std::unique_ptr<     module::Encoder_conv<>>(new      module::Encoder_conv<>(p.K, p.N, p.poly));
	...
	m.decoder = std::unique_ptr<     module::Decoder_Viterbi_list_parallel<int, float>>(new      module::Decoder_Viterbi_list_parallel<int, float>(p.K, p.N, p.L, *m.crc, trellis, false));
